...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loginForm.addEventListener('submit', async (e) => {
      ...
      if (window.firebaseAuth) {
        try {
          ...
          // Firestore membership lookup for cross-device mess persistence
          if (window.firestore) {
            try {
              console.log(`[Firestore Lookup] Checking for mess membership for email: ${u.email}`);
              ...
              if (!snap.empty) {
                console.log(`[Firestore Lookup] Found ${snap.docs.length} membership document(s).`);
                const data = snap.docs[0].data();
                console.log('[Firestore Lookup] Membership data:', data);
                const foundMessId = data.messId;
                const role = data.role || 'member';
                if (foundMessId) {
                  console.log(`[Firestore Lookup] Found messId: ${foundMessId}. Redirecting to dashboard.`);
                  localStorage.setItem('currentMess', JSON.stringify({ messId: foundMessId, role }));
                  window.location.href = 'index.html';
                  return;
                } else {
                   console.log('[Firestore Lookup] Document found, but no messId field in it.');
                }
              } else {
                console.log('[Firestore Lookup] No membership document found for this email.');
              }
            } catch (e) {
              console.error('Firestore membership lookup failed', e);
            }
          }
      ...
      // Fallback: localStorage-only implementation
      if (loginError) loginError.textContent = 'Firebase is not available. Cannot log in.';
    });
  }
  ...
  if (createMessForm) {
    createMessForm.addEventListener('submit', async (e) => {
      ...
      // Firestore-backed creation for cross-device persistence
      if (window.firestore) {
        try {
          const cu = (() => { try { return JSON.parse(localStorage.getItem('currentUser')); } catch { return null; } })();
          const messRef = window.firestore.collection('messes').doc(messId);
          const messSnap = await messRef.get();
          if (messSnap.exists) {
            if (createMessError) createMessError.textContent = 'Mess ID already exists.';
            return;
          }
          await messRef.set({ password: messPassword, createdBy: cu ? (cu.uid || cu.email || cu.id) : null, createdAt: Date.now() });
          await window.firestore.collection('members').add({ messId, email: cu ? cu.email : null, name: cu ? cu.name : '', role: 'manager', joinDate: new Date().toISOString().split('T')[0] });
          localStorage.setItem('currentMess', JSON.stringify({ messId, role: 'manager' }));
          window.location.href = 'index.html';
          return;
        } catch (err) {
          console.error('Firestore mess creation failed', err);
          if (createMessError) createMessError.textContent = err && err.message ? err.message : 'Failed to create mess.';
          return;
        }
      }
  ...
  if (joinMessForm) {
    joinMessForm.addEventListener('submit', async (e) => {
      ...
      // Firestore-backed join for cross-device persistence
      if (window.firestore) {
        try {
          const cuJoin = (() => { try { return JSON.parse(localStorage.getItem('currentUser')); } catch { return null; } })();
          const messRef = window.firestore.collection('messes').doc(messId);
          const messSnap = await messRef.get();
          if (!messSnap.exists) {
          ...
          if (!messData || messData.password !== messPassword) {
            if (joinMessError) joinMessError.textContent = 'Incorrect mess password.';
            return;
          }
          // Add membership
          await window.firestore.collection('members').add({ messId, email: cuJoin ? cuJoin.email : null, name: cuJoin ? cuJoin.name : '', role: 'member', joinDate: new Date().toISOString().split('T')[0] });
          localStorage.setItem('currentMess', JSON.stringify({ messId, role: 'member' }));
          window.location.href = 'index.html';
          return;
        } catch (err) {
          console.error('Firestore join failed', err);
          if (joinMessError) joinMessError.textContent = err && err.message ? err.message : 'Failed to join mess.';
          return;
        }
      }
